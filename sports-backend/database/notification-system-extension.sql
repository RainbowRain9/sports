...
('registration_approve_email', '报名批准-邮件通知', 'email', '【嘉园运动会系统】报名审核通过通知', '<div>亲爱的 {{player_name}} 同学：<br><br>恭喜您！您报名的项目"{{schedule_name}}"已通过审核。<br><br>比赛时间：{{schedule_date}} {{schedule_starttime}}<br>{{#note}}审核备注：{{note}}<br>{{/note}}<br>请按时参加比赛，祝您取得好成绩！</div>', '{"player_name":"运动员姓名","schedule_name":"项目名称","schedule_date":"比赛日期","schedule_starttime":"开始时间","note":"审核备注"}'),
('registration_reject_email', '报名拒绝-邮件通知', 'email', '【嘉园运动会系统】报名审核结果通知', '<div>亲爱的 {{player_name}} 同学：<br><br>很遗憾，您报名的项目"{{schedule_name}}"未通过审核。<br><br>{{#note}}审核备注：{{note}}<br>{{/note}}<br>如有疑问，请联系管理员。</div>', '{"player_name":"运动员姓名","schedule_name":"项目名称","note":"审核备注"}'),
