...
-- 扩展player表
ALTER TABLE `player` 
ADD COLUMN IF NOT EXISTS `player_email` varchar(100) DEFAULT NULL COMMENT '邮箱地址',
ADD COLUMN IF NOT EXISTS `player_phone` varchar(20) DEFAULT NULL COMMENT '手机号码',
ADD COLUMN IF NOT EXISTS `player_status` tinyint(1) DEFAULT 1 COMMENT '状态：1-正常，0-禁用',
...
-- 扩展admin表
ALTER TABLE `admin` 
ADD COLUMN IF NOT EXISTS `admin_email` varchar(100) DEFAULT NULL COMMENT '邮箱地址',
ADD COLUMN IF NOT EXISTS `admin_phone` varchar(20) DEFAULT NULL COMMENT '手机号码',
ADD COLUMN IF NOT EXISTS `admin_status` tinyint(1) DEFAULT 1 COMMENT '状态：1-正常，0-禁用',
ADD COLUMN IF NOT EXISTS `notification_settings` json DEFAULT NULL COMMENT '通知设置',
ADD COLUMN IF NOT EXISTS `created_at` timestamp DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
ADD COLUMN IF NOT EXISTS `updated_at` timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间';
...
-- 扩展judge表
ALTER TABLE `judge` 
ADD COLUMN IF NOT EXISTS `judge_email` varchar(100) DEFAULT NULL COMMENT '邮箱地址',
ADD COLUMN IF NOT EXISTS `judge_phone` varchar(20) DEFAULT NULL COMMENT '手机号码',
ADD COLUMN IF NOT EXISTS `judge_level` varchar(20) DEFAULT NULL COMMENT '裁判等级',
ADD COLUMN IF NOT EXISTS `judge_certificate` varchar(100) DEFAULT NULL COMMENT '证书编号',
ADD COLUMN IF NOT EXISTS `notification_settings` json DEFAULT NULL COMMENT '通知设置';

-- 扩展competition表
ALTER TABLE `competition` 
ADD COLUMN IF NOT EXISTS `competition_location` varchar(200) DEFAULT NULL COMMENT '比赛地点',
ADD COLUMN IF NOT EXISTS `competition_rules` text DEFAULT NULL COMMENT '比赛规则',
ADD COLUMN IF NOT EXISTS `competition_status` tinyint(1) DEFAULT 1 COMMENT '状态：1-正常，2-取消，3-延期',
ADD COLUMN IF NOT EXISTS `weather_requirement` varchar(100) DEFAULT NULL COMMENT '天气要求';

-- 8. 添加关键索引
-- registration表索引
CREATE INDEX IF NOT EXISTS `idx_registration_player_status` ON `registration`(`player_id`, `registration_status`);
CREATE INDEX IF NOT EXISTS `idx_registration_schedule_status` ON `registration`(`schedule_id`, `registration_status`);
CREATE INDEX IF NOT EXISTS `idx_registration_time` ON `registration`(`registration_time`);

-- plog表索引
CREATE INDEX IF NOT EXISTS `idx_plog_player_schedule` ON `plog`(`plog_playerid`, `plog_scheduleid`);
CREATE INDEX IF NOT EXISTS `idx_plog_schedule_score` ON `plog`(`plog_scheduleid`, `plog_score`);

-- competition表索引
CREATE INDEX IF NOT EXISTS `idx_competition_date_status` ON `competition`(`schedule_date`, `registration_status`);
CREATE INDEX IF NOT EXISTS `idx_competition_type_date` ON `competition`(`schedule_itemid`, `schedule_date`);

-- player表索引
CREATE INDEX IF NOT EXISTS `idx_player_class_status` ON `player`(`player_class`, `player_status`);
CREATE INDEX IF NOT EXISTS `idx_player_email` ON `player`(`player_email`);
...
SELECT 
  ...
  (SELECT COUNT(*) FROM admin WHERE admin_status = 1) as active_admins,
  ...
  (SELECT COUNT(*) FROM system_notification WHERE status = 0) as unread_notifications;
...
SELECT 
  ...
  COUNT(DISTINCT c.schedule_itemid) as event_types
FROM player p
LEFT JOIN plog pl ON p.player_id = pl.plog_playerid
LEFT JOIN competition c ON pl.plog_scheduleid = c.schedule_id
